...
//        mPaint.setAlpha(20); // 设置alpha不透明度,范围为0~255
//        mPaint.setAntiAlias(true); // 抗锯齿
        mPaint.setStyle(Paint.Style.FILL); //描边效果 1.FILL(填充效果) 2.STROKE(描边效果) 3.FILL_AND_STROKE(填充+描边)
        mPaint.setStrokeWidth(50);//描边宽度
//        mPaint.setStrokeCap(Paint.Cap.ROUND); //圆角效果 线的末尾的形状 1.BUTT() 2.ROUND() 3.SQUARE()
 ...
 * tile:用于指定控件区域大于指定的渐变区域时，空白区域的颜色填充方法   1.CLAMP(以边缘最后一个像素填充) 2.REPEAT(重复填充) 3.MIRROR(镜像填充)
 *//*
...
*/
         ...
         *//*
        mShader = new RadialGradient(250, 250, 125, new int[]{Color.GREEN, Color.YELLOW, Color.RED}, null, Shader.TileMode.MIRROR);
        mPaint.setShader(mShader);
...
//         */
//        mShader = new SweepGradient(250, 250, new int[]{Color.RED, Color.GREEN, Color.RED}, new float[]{0, 0.5f, 1});
//        mPaint.setShader(mShader);
//        canvas.drawCircle(250, 250, 250, mPaint);
